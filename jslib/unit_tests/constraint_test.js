...
unit.test("constraint test: checkConstraintsAssembly", async () => {
    ...
    assert(~ref_integer["all_of"].indexOf("this.state.data.int3==ref.state.data.int3"));
    assert(~ref_integer["all_of"].indexOf("this.definition.data.int1!=ref.state.data.int4"));
    assert(~ref_integer["all_of"].indexOf("this.definition.data.int2<ref.state.data.int3"));
    assert(~ref_integer["all_of"].indexOf("this.definition.data.int1<=ref.definition.data.int2"));
    assert(~ref_integer["all_of"].indexOf("this.state.data.int4>=ref.definition.data.int1"));
    assert(~ref_integer["all_of"].indexOf("this.state.revision!=ref.state.revision"));
    assert(~ref_integer["all_of"].indexOf("this.state.revision<ref.state.revision"));
    assert(~ref_integer["all_of"].indexOf("this.state.revision<ref.state.data.float3"));
    assert(~ref_integer["all_of"].indexOf("ref.state.revision<this.state.data.double2"));
    assert(~ref_integer["all_of"].indexOf("ref.state.revision==4"));
    assert(~ref_integer["all_of"].indexOf("ref.state.revision defined"));
    assert(~ref_integer["all_of"].indexOf("ref.state.revision!=null"));
    assert(~ref_integer["all_of"].indexOf("ref.definition.data.int1 defined"));
    assert(~ref_integer["all_of"].indexOf("ref.definition.data.int3 undefined"));
    assert(~ref_integer["all_of"].indexOf("ref.definition.data.int4==null"));
    assert(~ref_integer["all_of"].indexOf("ref.state.data.int4 defined"));
    assert(~ref_integer["all_of"].indexOf("ref.state.data.int3!=null"));
    assert(~ref_integer["all_of"].indexOf("ref.state.data.int2 undefined"));
    assert(~ref_integer["all_of"].indexOf("this.definition.data.int2==ref.definition.data.float2"));
    assert(~ref_integer["all_of"].indexOf("this.state.data.int3>ref.definition.data.float1"));
    assert(~ref_integer["all_of"].indexOf("this.definition.data.int2<ref.state.data.float3"));
    assert(~ref_integer["all_of"].indexOf("this.state.data.int3<=ref.state.data.float4"));
    assert(~ref_integer["all_of"].indexOf("this.definition.data.int2>=ref.definition.data.float2"));
    assert(~ref_integer["all_of"].indexOf("ref.definition.data.int1>6"));
    assert(~ref_integer["all_of"].indexOf("ref.state.data.int4<1000000.0"));
    assert(~ref_integer["all_of"].indexOf("ref.definition.data.int2>=50"));
    assert(~ref_integer["all_of"].indexOf("ref.state.data.int3<=777"));
    assert(~ref_integer["all_of"].indexOf("ref.definition.data.int2==77.0"));
    assert(~ref_integer["all_of"].indexOf("ref.state.data.int4!=1000"));
    assert(~ref_integer["all_of"].indexOf("this.state.data.bigdecimal3::number==ref.state.data.bigdecimal3::number"));
    assert(~ref_integer["all_of"].indexOf("this.state.data.bigdecimal3::number!=ref.state.data.bigdecimal4::number"));
    assert(~ref_integer["all_of"].indexOf("ref.state.data.bigdecimal3::number==54080961345783"));
    assert(~ref_integer["all_of"].indexOf("ref.state.data.bigdecimal4::number==143434433230090.0"));
    assert(~ref_integer["all_of"].indexOf("ref.state.data.bigdecimal4::number!=143434433230091"));
    assert(~ref_integer["all_of"].indexOf("ref.state.data.bigdecimal3::number!=ref.state.data.float3"));
    assert(~ref_integer["all_of"].indexOf("ref.state.data.bigdecimal4::number!=-3902932093200002020"));
    assert(~ref_integer["all_of"].indexOf("this.state.data.bigdecimal4::number>ref.state.data.bigdecimal3::number"));
    assert(~ref_integer["all_of"].indexOf("ref.state.data.bigdecimal4::number>=this.state.data.bigdecimal3::number"));
    assert(~ref_integer["all_of"].indexOf("this.state.data.bigdecimal3::number<=ref.state.data.bigdecimal3::number"));
    ...
    assert(~ref_float["all_of"].indexOf("this.state.data.float3==ref.state.data.float3"));
    assert(~ref_float["all_of"].indexOf("this.definition.data.float1!=ref.state.data.float4"));
    assert(~ref_float["all_of"].indexOf("this.state.data.float3>ref.definition.data.float2"));
    assert(~ref_float["all_of"].indexOf("this.definition.data.float2<ref.state.data.float3"));
    assert(~ref_float["all_of"].indexOf("this.definition.data.float1<=ref.definition.data.float2"));
    assert(~ref_float["all_of"].indexOf("this.state.data.float4>=ref.definition.data.float1"));
    assert(~ref_float["all_of"].indexOf("ref.definition.data.float1 defined"));
    assert(~ref_float["all_of"].indexOf("ref.definition.data.float2!=null"));
    assert(~ref_float["all_of"].indexOf("ref.definition.data.float3 undefined"));
    assert(~ref_float["all_of"].indexOf("ref.state.data.float4 defined"));
    assert(~ref_float["all_of"].indexOf("ref.state.data.float2 undefined"));
    assert(~ref_float["all_of"].indexOf("ref.state.data.float1==null"));
    assert(~ref_float["all_of"].indexOf("this.definition.data.float2==ref.definition.data.float2"));
    assert(~ref_float["all_of"].indexOf("this.definition.data.float1==ref.definition.data.int1"));
    assert(~ref_float["all_of"].indexOf("this.state.data.float3>ref.definition.data.int1"));
    assert(~ref_float["all_of"].indexOf("this.definition.data.float2<ref.state.data.int3"));
    assert(~ref_float["all_of"].indexOf("this.state.data.float3<=ref.state.data.int4"));
    assert(~ref_float["all_of"].indexOf("this.definition.data.float2>=ref.definition.data.int2"));
    assert(~ref_float["all_of"].indexOf("ref.definition.data.float1>6.78"));
    assert(~ref_float["all_of"].indexOf("ref.state.data.float4<1000000"));
    assert(~ref_float["all_of"].indexOf("ref.definition.data.float2>=-58.037092"));
    assert(~ref_float["all_of"].indexOf("ref.state.data.float3<=77780192993439232.2389283928"));
    assert(~ref_float["all_of"].indexOf("ref.definition.data.float2==77"));
    assert(~ref_float["all_of"].indexOf("ref.state.data.float4!=1000.0"));
    assert(~ref_float["all_of"].indexOf("this.state.data.double1==ref.state.data.double1"));
    assert(~ref_float["all_of"].indexOf("this.state.data.double1!=ref.state.data.double2"));
    assert(~ref_float["all_of"].indexOf("ref.state.data.double1==-93029039209309103.09204932042000024555"));
    assert(~ref_float["all_of"].indexOf("ref.state.data.double2==3232322288829209309103.09204932042000024555"));
    assert(~ref_float["all_of"].indexOf("ref.state.data.double1!=ref.state.data.float4"));
    assert(~ref_float["all_of"].indexOf("ref.state.data.double2!=-390293209320.329029302998487300020204"));
    assert(~ref_float["all_of"].indexOf("this.state.data.double2>ref.state.data.double1"));
    assert(~ref_float["all_of"].indexOf("ref.state.data.double2>=this.state.data.double1"));
    assert(~ref_float["all_of"].indexOf("this.state.data.double1<=ref.state.data.double1"));
    ...
    assert(~ref_string["all_of"].indexOf("this.definition.data.string1==ref.definition.data.string1"));
    assert(~ref_string["all_of"].indexOf("this.state.data.string3!=ref.state.data.string4"));
    assert(~ref_string["all_of"].indexOf("this.state.branchId==\"1:25\""));
    assert(~ref_string["all_of"].indexOf("this.state.branchId!=\"1:26\""));
    assert(~ref_string["all_of"].indexOf("this.state.branchId defined"));
    assert(~ref_string["all_of"].indexOf("this.state.branchId!=null"));
    assert(~ref_string["all_of"].indexOf("ref.definition.data.string1 defined"));
    assert(~ref_string["all_of"].indexOf("ref.definition.data.string2!=null"));
    assert(~ref_string["all_of"].indexOf("ref.state.data.string2 undefined"));
    assert(~ref_string["all_of"].indexOf("ref.state.data.string1==null"));
    assert(~ref_string["all_of"].indexOf("ref.state.data.string4 defined"));
    assert(~ref_string["all_of"].indexOf("ref.definition.data.string3 undefined"));
    assert(~ref_string["all_of"].indexOf("ref.definition.data.string2==\"==INFORMATION==\""));
    assert(~ref_string["all_of"].indexOf("ref.state.data.string3!=\"string\""));
    assert(~ref_string["all_of"].indexOf("ref.state.branchId undefined"));
    ...
    assert(~ref_boolean["all_of"].indexOf("this.definition.data.boolean2==ref.definition.data.boolean2"));
    assert(~ref_boolean["all_of"].indexOf("this.state.data.boolean4!=ref.definition.data.boolean1"));
    assert(~ref_boolean["all_of"].indexOf("ref.definition.data.boolean2 defined"));
    assert(~ref_boolean["all_of"].indexOf("ref.state.data.boolean1 undefined"));
    assert(~ref_boolean["all_of"].indexOf("ref.state.data.boolean2==null"));
    assert(~ref_boolean["all_of"].indexOf("ref.state.data.boolean3 defined"));
    assert(~ref_boolean["all_of"].indexOf("ref.state.data.boolean3!=null"));
    assert(~ref_boolean["all_of"].indexOf("ref.definition.data.boolean4 undefined"));
    assert(~ref_boolean["all_of"].indexOf("ref.definition.data.boolean1==true"));
    assert(~ref_boolean["all_of"].indexOf("ref.definition.data.boolean2==false"));
    assert(~ref_boolean["all_of"].indexOf("ref.state.data.boolean3!=false"));
    ...
    assert(~ref_inherited["all_of"].indexOf("inherits this.definition.references.ref_roles"));
    assert(~ref_inherited["all_of"].indexOf("inherits this.definition.references.ref_float"));
    assert(~ref_inherited["all_of"].indexOf("inherits this.definition.references.ref_string"));
    assert(~ref_inherited["all_of"].indexOf("inherits this.definition.references.ref_boolean"));
    assert(~ref_inherited["all_of"].indexOf("this.definition.references.ref_roles defined"));
    assert(~ref_inherited["all_of"].indexOf("ref.state.references.ref_roles undefined"));
    assert(~ref_inherited["all_of"].indexOf("this.state.references.ref_inherited is_a this.definition.references.ref_float"));
    ...
    assert(~ref_time["all_of"].indexOf("this.definition.expires_at>0"));
    assert(~ref_time["all_of"].indexOf("this.definition.created_at>10000"));
    assert(~ref_time["all_of"].indexOf("this.definition.expires_at<\"4908-04-18 23:58:00\""));
    assert(~ref_time["all_of"].indexOf("this.definition.expires_at>this.definition.data.time1"));
    assert(~ref_time["all_of"].indexOf("this.definition.data.time2>=ref.state.created_at"));
    assert(~ref_time["all_of"].indexOf("this.definition.data.time4>=ref.definition.expires_at"));
    assert(~ref_time["all_of"].indexOf("this.definition.data.time5<=ref.definition.created_at"));
    assert(~ref_time["all_of"].indexOf("this.definition.data.int2<=ref.definition.created_at"));
    assert(~ref_time["all_of"].indexOf("this.state.data.time5<this.state.data.time6"));
    assert(~ref_time["all_of"].indexOf("this.definition.expires_at!=this.definition.created_at"));
    assert(~ref_time["all_of"].indexOf("this.definition.created_at>=ref.definition.data.time2"));
    assert(~ref_time["all_of"].indexOf("this.definition.created_at<ref.state.data.time3"));
    assert(~ref_time["all_of"].indexOf("this.definition.created_at>ref.definition.data.time4"));
    assert(~ref_time["all_of"].indexOf("this.definition.expires_at!=ref.definition.data.time5"));
    assert(~ref_time["all_of"].indexOf("this.definition.expires_at!=ref.state.data.time6"));
    assert(~ref_time["all_of"].indexOf("this.definition.expires_at<ref.definition.data.bigdecimal4::number"));
    assert(~ref_time["all_of"].indexOf("this.definition.expires_at>ref.state.data.time5"));
    assert(~ref_time["all_of"].indexOf("this.state.data.time2>0"));
    assert(~ref_time["all_of"].indexOf("this.state.created_at>10000"));
    assert(~ref_time["all_of"].indexOf("this.definition.data.active_since>this.definition.data.time1"));
    assert(~ref_time["all_of"].indexOf("this.state.data.time2>=ref.state.created_at"));
    assert(~ref_time["all_of"].indexOf("this.state.data.time4>=ref.definition.expires_at"));
    assert(~ref_time["all_of"].indexOf("this.state.data.time5<=ref.definition.created_at"));
    assert(~ref_time["all_of"].indexOf("this.state.expires_at>=ref.definition.created_at"));
    assert(~ref_time["all_of"].indexOf("ref.definition.expires_at<=100000000000"));
    assert(~ref_time["all_of"].indexOf("ref.definition.created_at>\"1986-01-18 17:00:10\""));
    assert(~ref_time["all_of"].indexOf("ref.definition.created_at>this.definition.data.time3"));
    assert(~ref_time["all_of"].indexOf("ref.definition.expires_at<this.definition.data.time6"));
    assert(~ref_time["all_of"].indexOf("ref.definition.created_at>=this.definition.created_at"));
    assert(~ref_time["all_of"].indexOf("ref.definition.expires_at>ref.state.created_at"));
    assert(~ref_time["all_of"].indexOf("ref.definition.created_at<ref.definition.expires_at"));
    assert(~ref_time["all_of"].indexOf("ref.definition.created_at>ref.state.data.int3"));
    assert(~ref_time["all_of"].indexOf("ref.definition.created_at==ref.state.created_at"));
    assert(~ref_time["all_of"].indexOf("ref.definition.created_at<=this.definition.data.time4"));
    assert(~ref_time["all_of"].indexOf("ref.state.data.time5<ref.state.data.time3"));
    assert(~ref_time["all_of"].indexOf("ref.state.expires_at>=this.definition.created_at"));
    assert(~ref_time["all_of"].indexOf("ref.definition.data.time2<=ref.state.created_at"));
    assert(~ref_time["all_of"].indexOf("this.state.data.time3!=ref.state.data.time3"));
    assert(~ref_time["all_of"].indexOf("now>this.definition.data.time1"));
    assert(~ref_time["all_of"].indexOf("now<=ref.state.data.time3"));
    assert(~ref_time["all_of"].indexOf("now==now"));
    assert(~ref_time["all_of"].indexOf("now<100000000000000000"));
    assert(~ref_time["all_of"].indexOf("now<this.definition.expires_at"));
    assert(~ref_time["all_of"].indexOf("now>\"1968-04-18 23:58:01\""));
    assert(~ref_time["all_of"].indexOf("now!=\"2086-03-22 11:35:37\""));
    assert(~ref_time["all_of"].indexOf("now>=600000"));
    assert(~ref_time["all_of"].indexOf("now<this.definition.expires_at"));
    assert(~ref_time["all_of"].indexOf("now>\"1968-04-18 23:58:01\""));
    assert(~ref_time["all_of"].indexOf("now!=\"2086-03-22 11:35:37\""));

    let ref_hashes = contract1.constraints.get("ref_hashes").exportConditions();
    assert(~ref_hashes["all_of"].indexOf("this.state.data.contract2_origin==ref.state.origin"));
    assert(~ref_hashes["all_of"].indexOf("this.id defined"));
    assert(~ref_hashes["all_of"].indexOf("this.id!=ref.id"));
    assert(~ref_hashes["all_of"].indexOf("this.id!=\"EAewHEvdkUJY4UQOB+OAF+V3zdMyryXBCnVtPfqZDUTUX2iUaldvl09/4etdb7ArZrVyP+Iq3E31tSUtm8dAOpxxwocEDybiFa/DCLKFmvv3ELsBc/vNMU+lTJ8XQQpb\""));
    assert(~ref_hashes["all_of"].indexOf("this.id!=this.state.data.string5"));
    assert(~ref_hashes["all_of"].indexOf("this.state.data.contract2_id==ref.id"));
    assert(~ref_hashes["all_of"].indexOf("ref.state.origin==this.state.data.contract2_origin"));
    assert(~ref_hashes["all_of"].indexOf("ref.state.origin!=\"oRhAix5Z9/O1MCjyJ/Iz1xHB2h8mcSC6Ylj//O06xJr2xeJF/XmE9nQpP7Ar9COlgsIP6QHhQcRltYlNdtDx/154bzaGGNA3FUqll/0aOM4qikYbqTrwCoKlTQIlaRI3\""));
    assert(~ref_hashes["all_of"].indexOf("this.state.data.contract2_origin==ref.state.origin"));
    assert(~ref_hashes["all_of"].indexOf("ref.state.origin defined"));
    assert(~ref_hashes["all_of"].indexOf("ref.state.origin==this.state.data.contract2_origin"));
    assert(~ref_hashes["all_of"].indexOf("ref.id!=this.id"));
    assert(~ref_hashes["all_of"].indexOf("this.state.data.string5!=this.id"));
    ...
    assert(~ref_bigdecimal["all_of"].indexOf("this.state.data.bigdecimal1::number>1000"));
    assert(~ref_bigdecimal["all_of"].indexOf("ref.state.data.bigdecimal2::number>=1000000000000000000000000000000000000000000000000"));
    assert(~ref_bigdecimal["all_of"].indexOf("ref.state.data.bigdecimal1::number<\"490392893427849819381293782734974325843959438758947358943789579287349817487198743892718937289378938\""));
    assert(~ref_bigdecimal["all_of"].indexOf("this.state.data.bigdecimal1::number<=90540330931039888203498939009091424678928328433888839898000111129094320492094029007845298372939"));
    assert(~ref_bigdecimal["all_of"].indexOf("ref.state.data.bigdecimal1::number>\"-10000000000000000000\""));
    assert(~ref_bigdecimal["all_of"].indexOf("1000<this.state.data.bigdecimal1::number"));
    assert(~ref_bigdecimal["all_of"].indexOf("1000000000000000000000000000000000000000000000000<=ref.state.data.bigdecimal2::number"));
    assert(~ref_bigdecimal["all_of"].indexOf("490392893427849819381293782734974325843959438758947358943789579287349817487198743892718937289378938>ref.state.data.bigdecimal1::number"));
    assert(~ref_bigdecimal["all_of"].indexOf("90540330931039888203498939009091424678928328433888839898000111129094320492094029007845298372939>=this.state.data.bigdecimal1::number"));
    assert(~ref_bigdecimal["all_of"].indexOf("-10000000000000000000<ref.state.data.bigdecimal1::number"));
    assert(~ref_bigdecimal["all_of"].indexOf("this.state.data.bigdecimal1::number>this.state.data.bigdecimal2::number"));
    assert(~ref_bigdecimal["all_of"].indexOf("ref.state.data.bigdecimal2::number<ref.state.data.bigdecimal1::number"));
    assert(~ref_bigdecimal["all_of"].indexOf("this.state.data.bigdecimal1::number>=ref.state.data.bigdecimal2::number"));
    assert(~ref_bigdecimal["all_of"].indexOf("this.state.data.bigdecimal1::number>this.state.data.bigdecimal2"));
    assert(~ref_bigdecimal["all_of"].indexOf("ref.state.data.bigdecimal2<ref.state.data.bigdecimal1::number"));
    assert(~ref_bigdecimal["all_of"].indexOf("this.state.data.bigdecimal1::number>=ref.state.data.bigdecimal2"));
    assert(~ref_bigdecimal["all_of"].indexOf("this.state.data.bigdecimal1::number==90540330931039888203498939009091424678928328433888839898000111129094320492094029007845298372939"));
    assert(~ref_bigdecimal["all_of"].indexOf("ref.state.data.bigdecimal1::number==\"490392893427849819381293782734974325843959438758947358943789579287349817487198743892718937289378937\""));
    assert(~ref_bigdecimal["all_of"].indexOf("ref.state.data.bigdecimal1::number!=490392893427849819381293782734974325843959438758947358943789579287349817487198743892718937343"));
    assert(~ref_bigdecimal["all_of"].indexOf("this.state.data.bigdecimal1::number!=\"83584938594\""));
    assert(~ref_bigdecimal["all_of"].indexOf("90540330931039888203498939009091424678928328433888839898000111129094320492094029007845298372939==this.state.data.bigdecimal1::number"));
    assert(~ref_bigdecimal["all_of"].indexOf("490392893427849819381293782734974325843959438758947358943789579287349817487198743892718937289378937==ref.state.data.bigdecimal1::number"));
    assert(~ref_bigdecimal["all_of"].indexOf("490392893427849819381293782734974325843959438758947358943789579287349817487198743892718937343!=ref.state.data.bigdecimal1::number"));
    assert(~ref_bigdecimal["all_of"].indexOf("83584938594!=this.state.data.bigdecimal1::number"));
    assert(~ref_bigdecimal["all_of"].indexOf("ref.state.data.bigdecimal2::number==this.state.data.bigdecimal1::number"));
    assert(~ref_bigdecimal["all_of"].indexOf("ref.state.data.bigdecimal2::number!=this.state.data.bigdecimal2::number"));
    assert(~ref_bigdecimal["all_of"].indexOf("ref.state.data.bigdecimal2==this.state.data.bigdecimal1::number"));
    ...
    assert(~ref_parent["all_of"].indexOf("ref.state.parent==this.state.data.contract3_parent"));
    assert(~ref_parent["all_of"].indexOf("ref.state.parent!=\"oRhAix5Z9/O1MCjyJ/Iz1xHB2h8mcSC6Ylj//O06xJr2xeJF/XmE9nQpP7Ar9COlgsIP6QHhQcRltYlNdtDx/154bzaGGNA3FUqll/0aOM4qikYbqTrwCoKlTQIlaRI3\""));
    assert(~ref_parent["all_of"].indexOf("this.state.data.contract3_parent==ref.state.parent"));
    assert(~ref_parent["all_of"].indexOf("ref.state.parent defined"));
    assert(~ref_parent["all_of"].indexOf("ref.state.parent!=this.state.data.string5"));
    assert(~ref_parent["all_of"].indexOf("this.state.data.string5!=ref.state.parent"));
    ...
    assert(~ref_can_play["all_of"].indexOf("this can_play ref.issuer"));
