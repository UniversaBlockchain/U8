...
struct TLS_data {
    evt_ctx_t* TLScontext;
    uv_tls_t* tls;
};

struct external_TLS_data {
    struct TLS_data* tls_data;
};

static uv_tls_t* getTLSFromData(void* data) {
    return ((struct external_TLS_data*) data)->tls_data->tls;
}
...
int uv_tls_init(evt_ctx_t *ctx, uv_tcp_t *tcp, uv_tls_t *endpt)
{
    ...
    if (!t)
        return ERR_TLS_GET_TLS_SESSION;
...
{
    uv_tls_t *utls = getTLSFromData(handle->data);
...
static void on_hd_complete( evt_tls_t *t, int status)
{
    ...
    uv_read_stop((uv_stream_t*) ut->tcp_hdl);
...
int uv_tls_read(uv_tls_t *tls, uv_tls_read_cb cb)
{
    ...
    int res = uv_read_start((uv_stream_t*) ptr->tcp_hdl, alloc_cb, on_tcp_read);
    evt_tls_read(ptr->tls, evt_on_rd);
    return res;
}
